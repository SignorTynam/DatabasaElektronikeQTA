...
if ($group_id === null || $group_id <= 0) { http_response_code(400); echo json_encode(['ok'=>false,'error'=>'ID grupi i pavlefshëm.']); exit; }
...
try {
  ...
  if ($action==='update_group_end') {
    ...
    if (!empty($G['exam_date']) && $G['exam_date'] < $end) throw new RuntimeException('Data e testit duhet të jetë ≥ datës së mbarimit. Ndrysho testin më parë nëse duhet.');
  ...
  if ($action==='update_exam_date') {
    ...
    if (!empty($G['end_date']) && $exam < $G['end_date']) throw new RuntimeException('Data e testit duhet të jetë ≥ datës së mbarimit të kursit.');
  ...
  if ($action==='update_final_score') {
    ...
    if (!$chk->fetchColumn()) throw new RuntimeException('Ky student nuk i përket këtij grupi.');
