...
if (!$currentUser || strtolower((string)($currentUser['role_name'] ?? ''))!=='editor') { header('Location: selectProfile.php'); exit; }

/* ===== Helpers ===== */
function h(?string $s): string { return htmlspecialchars($s ?? '', ENT_QUOTES, 'UTF-8'); }
function intv($v): int { return (int)$v; }

/* ===== Core KPIs (pa logs, pa nota) ===== */
$studentsTotal   = (int)$pdo->query("SELECT COUNT(*) FROM students")->fetchColumn();
$companiesTotal  = (int)$pdo->query("SELECT COUNT(*) FROM agencies")->fetchColumn(); // UI: Kompanitë
...
/* ===== Distributions & Trends (no audit) ===== */

/* Funneli (students lifecycle) */
...
$studentsInActiveGroups = (int)$pdo->query("
  ...
  WHERE CURDATE() BETWEEN cg.start_date AND cg.end_date
")->fetchColumn();
$studentsWithCompany = (int)$pdo->query("SELECT COUNT(DISTINCT student_id) FROM agency_students")->fetchColumn();
...
/* Groups timeline (last 14 days): starts vs exams — replaces audit chart */
$cgSeriesRows = $pdo->query("
  SELECT d,
         SUM(start_cnt) AS starts,
         SUM(exam_cnt)  AS exams
  FROM (
    SELECT DATE(start_date) AS d, 1 AS start_cnt, 0 AS exam_cnt
    FROM course_groups
    WHERE start_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 14 DAY) AND CURDATE()
    UNION ALL
    SELECT DATE(exam_date) AS d, 0, 1
    FROM course_groups
    WHERE exam_date IS NOT NULL
      AND exam_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 14 DAY) AND CURDATE()
  ) x
  GROUP BY d
  ORDER BY d ASC
")->fetchAll(PDO::FETCH_ASSOC);
...
/* Top companies by assigned students */
$companyTop = $pdo->query("
...
/* CG stacked */
$days = array_map(fn($r)=>$r['d'], $cgSeriesRows);
$starts = array_map(fn($r)=> (int)$r['starts'], $cgSeriesRows);
$exams  = array_map(fn($r)=> (int)$r['exams'],  $cgSeriesRows);
...
$funnel = [
  ...
  'Me kompani të caktuar'    => $studentsWithCompany,
];

/* Navbar i editorit (pa Logs) */
...
<head>
  <meta charset="UTF-8" />
  <title>Dashboard i Ri — Editor • QTA</title>
...
<main class="container-fluid px-3 px-md-4">
  ...
  <section class="hero p-4 p-md-5 mb-4">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
      ...
      <div class="soft">
        ...
        <div class="h5 mb-0"><?= h($currentUser['full_name'] ?: ($currentUser['email'] ?? 'Editor')) ?></div>
      </div>
    </div>
  </section>
  ...
  <section class="row g-4 mb-4 kpi">
    ...
    <div class="col-12 col-md-6 col-xl-3">
      <div class="card p-3 h-100">
        <div class="d-flex align-items-center">
          <div class="icon me-3"><i class="bi bi-buildings fs-4 text-danger"></i></div>
          <div>
            <div class="small text-muted text-uppercase">Kompanitë</div>
            <div class="h3 mb-1"><?= number_format($companiesTotal) ?></div>
            <span class="small text-muted">Përdorues: <?= number_format($usersTotal) ?></span>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="row g-4 mb-4">
    ...
    <div class="col-12 col-xl-5">
      <div class="card h-100">
        <div class="card-header bg-white d-flex align-items-center justify-content-between">
          <h5 class="mb-0"><i class="bi bi-calendar2-week me-2"></i>Grupet (14 ditë)</h5>
          <span class="text-muted small">Fillime vs Teste</span>
        </div>
        <div class="card-body">
          <canvas id="chartCG" height="120"></canvas>
        </div>
      </div>
    </div>
  </section>
    ...
    <div class="col-12 col-xl-5">
      <div class="card h-100">
        ...
        <div class="card-body">
          ...
          <?php else: ?>
            <p class="text-muted mb-0">S’ka të dhëna për kompani.</p>
          <?php endif; ?>
        </div>
      </div>
    </div>
...
const cgDays   = <?= json_encode($days) ?>;
const cgStarts = <?= json_encode($starts) ?>;
const cgExams  = <?= json_encode($exams) ?>;
...
(() => {
  ...
  const ccg = document.getElementById('chartCG');
  if (ccg) new Chart(ccg, {
    type: 'bar',
    data: {
      labels: cgDays,
      datasets: [
        { label: 'Fillime', data: cgStarts, stack:'x' },
        { label: 'Teste',   data: cgExams,  stack:'x' }
      ]
    },
